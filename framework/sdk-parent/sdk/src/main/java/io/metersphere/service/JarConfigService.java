...
import io.metersphere.base.mapper.ext.BaseJarConfigMapper;
...
public class JarConfigService {
    ...
    private static final String JAR_FILE_DIR = "/eyimu3/metersphere/data/jar";
    ...
    public Pager<List<JarConfig>> list(JarConfigRequest request, int pageNum, int pageSize) {
        ...
        return PageUtils.setPageInfo(page, baseJarConfigMapper.list(request));
    }
    ...
    public void buildQueryRequest(JarConfigRequest request) {
        ...
        if (request.getResourceType().equals(JarConfigResourceType.PROJECT.name())) {
            Project project = baseProjectService.getProjectById(request.getResourceId());
    ...
    private void checkExist(JarConfig jarConfig) {
        if (jarConfig.getName() != null) {
            ...
            buildQueryRequest(request);
            if (baseJarConfigMapper.checkExist(request) > 0) {
    ...
    public void initJarPath() {
        ...
        list.forEach(item -> {
            try {
                ...
                LogUtil.error(JSON.toJSONString(item));
                LogUtil.error(e);
            }
        });
